...
double Calculate_Complex_Percent_Function(double a, double y,int d ) {
       double pay = a * Math.pow((1 + y/ 12), 12 *d);
          return rnd(pay, 2);
  } double Calculate_Simple_Percent_Function1(double doubleAmount, double double_year_rate, int deposit_period) {
  ...
  ,int places) {
    ...
    void do_important_job( )
    {
      ...
      System.out.println   (   "Выберите тип вклада, 1 - вклад с обычным процентом, 2 - вклад с капитализацией:");
        ...
        if (action ==1) outDoubleVar = Calculate_Simple_Percent_Function1(amount, 0.06, period);
        else if (action == 2)
        {
